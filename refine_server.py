...
class RefineServer(object):
  ...
  def simple_quote(unsafe):
    if DEBUG:
      print "ORIGINAL : {0}".format(str(unsafe))
    quoted = quote_plus(unsafe)
    if DEBUG:
      print "QUOTED : {0}".format(str(quoted))
    return quoted
  ...
  def __unicode__(self):
    return ("{0} rows starting at {1} of {2} "
            "total rows ({3} filtered rows)\nSample Row:\n{4}".format(self.limit, self.offset, self.total_count,
                                                                      self.filtered_count,
                                                                      str(self.rows[0]) if self.total_count > 0 else "["
                                                                                                                     "no"
                                                                                                                     " "
                                                                                                                     "rows]"))
  ...
  def _get_import_job_status(self, job_id):
    ...
    if response:
      try:
        ...
        raise Exception("Failed to retrieve JSON from get-importing-job-status. {0}".format(e.message))
